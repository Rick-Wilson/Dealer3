//! Rust solver v2 (3-layer structure) with C++ solver-compatible file format and output
//!
//! Usage: cargo run --example solver_v2 --release -- -f <file>
//!
...
use dealer_dds::solver2::{get_node_count, set_xray_limit, set_no_pruning, set_no_tt, set_no_rank_skip, Hands, Solver};
...
fn main() {
    ...
    let file_path = match file_path {
        Some(p) => p,
        None => {
            eprintln!("Usage: solver_v2 -f <file> [-X <iterations>] [-P] [-T] [-R]");
            std::process::exit(1);
        }
    };
    ...
    // Solve for each trump/leader combination using solve_v2
    for &t in &trumps {
        ...
        if leaders.len() == 1 {
            ...
            let ns_tricks = solver.solve_v2();  // Use solve_v2
